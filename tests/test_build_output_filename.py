...
def test_preserves_accented_and_non_ascii_letters():
    assert build_output_filename("Ñuble", make_item()).startswith("Ñuble-")


def test_preserves_apostrophes():
    assert build_output_filename("O'Higgins", make_item()).startswith("O'Higgins-")


def test_replaces_spaces_with_underscore():
    assert build_output_filename("La Serena", make_item()).startswith("La_Serena-")


def test_replaces_single_nonprintable_character():
    filename = build_output_filename("foo\tbar", make_item())
    assert filename.startswith("foo_bar-")
...
def test_filename_shape_matches_slug_timestamp_hash_png():
    ...
    assert re.match(r"^Tongoy-20260716T145204-[0-9a-f]{8}\.png$", filename)
